        ...
        <div class="part">
            <div class="part-min">
                <div class="d-g gg-1 gaf-c jc-sb">
                    <div>
                        <a href="<?= APP_PREFIX; ?>/new-task" class="btn popup-link">Добавить задачу</a>
                    </div>
                    <div class="d-g gaf-c jc-l ac-c bg-2">
                        <?php foreach ($states as $stateKey => $stateName): ?>
                            <form method="post">
                                <input type="hidden" name="request" value="tasksChangeState">
                                <input type="hidden" name="redirect" value="<?=$_SERVER['REQUEST_URI'];?>">
                                <button type="submit" name="state" value="<?=$stateKey;?>" class="p-05 <?=$currentState == $stateKey ? 'bg-3' : 'bg-2';?>"><?=$stateName;?></button>
                            </form>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <!--<div class="bg-2">
                        <form method="post">
                            ...
                            <select name="executor" onchange="this.form.submit()" class="bg-2">
                ...
                <?php if($tasks && count($tasks) > 0) : ?>
                    ...
                    <div class="d-g gg-2 gtc-2">
                        <?php foreach ($tasks as $key => $task): ?>
                            ...
                            ?>
                            <a href="<?=APP_PREFIX;?>/task-<?=$task['id'];?>" class="popup-link bg-w p-1"
